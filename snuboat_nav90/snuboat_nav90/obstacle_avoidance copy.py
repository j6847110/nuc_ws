...
class Obstacle_Avoidance(Node):
    ...
    def __init__(self):
        ...
        self.wp_set_y_pub = self.create_publihser(Float64MultiArray, "wp_set/y", 1)
    ...
    def pub_des(self):
        ...
        err_g = [self.odom_pos[0]-self.odom_wp_x_set[self.cur_wp_idx],
               self.odom_pos[1]-self.odom_wp_y_set[self.cur_wp_idx],
               self.err_heading[-1]]
        
        #rotation matrix
        Rt = [[np.cos(self.heading[-1]), np.sin(self.heading[-1]),0],
              [-np.sin(self.heading[-1]),np.cos(self.heading[-1]),0],
              [0, 0, 1]]
        
        err_b = np.linalg.inv(Rt)*err_g

        temp_err_x = Float64()
        temp_err_x.data = err_b[0]
        temp_err_y = Float64()
        temp_err_y.data = err_b[1]
        ...
        if self.cur_wp_idx + 1 != len(self.odom_wp_x_set):
            ...
            err_heading_next_ = np.arctan2(self.odom_wp_y_set[self.cur_wp_idx+1] \
                                            - self.odom_pos[1], self.odom_wp_x_set[self.cur_wp_idx+1] - self.odom_pos[0]) - self.heading[-1]
            ...
            if len(self.obs_labels) != 0: # if there are scanned obstacles
                ...
                for i, idx in enumerate(idx_array):
                    ...
                    if idx != idx_array[-1]:
                        ...
                        if len(self.obs_r[idx:idx_array[i+1]-1]) > 0:
                            # print(1)
                            ...
                            print(self.obs_phi[idx:idx_array[i+1]-1])
                            if np.min(self.obs_r[idx:idx_array[i+1]-1]) < self.safe_radius:
                                # define start_phi
                                r = self.obs_r[idx]
                                inflate_obs_phi = np.arccos((2*r*r - (self.B * 0.5)**2)/(2*r*r))
                                del r
                                ...
                                inflate_obs_phi = np.arccos((2*r*r - (self.B * 0.5)**2)/(2*r*r)) # TODO: adjust parameters depending on dimension
                                del r
