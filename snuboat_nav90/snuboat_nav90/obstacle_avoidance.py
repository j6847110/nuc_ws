...
class Obstacle_Avoidance(Node):
    ...
    def cal_des(self):
        ...
        if self.wp_reach_check == True and self.wp_time_cnt < self.wp_stay_time:
            ...
            self.wp_reach_check = False
            self.cur_wp_idx +=1
            if self.cur_wp_idx > len(self.odom_wp_x_set):
                    ...
                    if idx != idx_array[-1]:
                        ...
                        if len(self.obs_r[idx:idx_array[i+1]-1]) > 10:
                            ...
                            if np.min(self.obs_r[idx:idx_array[i+1]-1]) < self.safe_radius:
                                # define start_phi
                                r = self.obs_r[idx]
                                inflate_obs_phi = np.arccos((2*r*r - (self.B * 0.7)**2)/(2*r*r))
                                del r
                                ...
                                inflate_obs_phi = np.arccos((2*r*r - (self.B * 0.7)**2)/(2*r*r)) # TODO: adjust parameters depending on dimension
                                del r
