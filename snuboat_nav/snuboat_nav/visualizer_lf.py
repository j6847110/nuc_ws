...
def euler_to_quaternion(roll, pitch, yaw):
    ...
    return [x, y, z, w]
def delete_values(array, values_to_delete):
    indices = np.isin(array, values_to_delete, invert=True)
    array = np.delete(array, np.where(indices))
    return array
...
class Visualizer(Node):
    ...
    #subscribe from imu
        ...
        self.viz_pose_pub = self.create_publisher(Marker, "/viz/lf/pose", 1)
        self.viz_wp1_pub = self.create_publisher(Marker, "/viz/lf/wp1", 1)
        self.viz_wp2_pub = self.create_publisher(Marker, "/viz/lf/wp2", 1)
        self.viz_wp3_pub = self.create_publisher(Marker, "/viz/lf/wp3", 1)
        self.viz_wp4_pub = self.create_publisher(Marker, "/viz/lf/wp4", 1)
        self.viz_des_heading_pub = self.create_publisher(Marker, "/viz/lf/des_heading", 1)
        self.viz_ref_heading_pub = self.create_publisher(Marker,"/viz/lf/ref_heading",1)
        ...
        self.safe_heading_sub = self.create_subscription(
            Float64MultiArray, "/safe_heading", self.safe_heading_callback, 1
        )
        ...
        self.viz_obs_phi_pub = self.create_publisher(MarkerArray, "/viz/lf/obs/phi", 1)
        self.viz_safe_heading_pub = self.create_publisher(MarkerArray,"/viz/lf/safe_heading",1)
        self.viz_danger_heading_pub = self.create_publisher(MarkerArray,"/viz/lf/danger_heading",1)
        self.obs_phi = []
        self.obs_r=[]
        self.des_heading = 0
        self.ref_heading=0
    ...
    def viz_pub(self):
        ...
        if(self.safe_heading_received==True):
            marker_array_msg = MarkerArray()
            #initial value

            for i in range(len(self.safe_heading)-1):
                curphi = self.safe_heading[i]
                color = [ 0.0,1.0,0.0]
                curphi_q = euler_to_quaternion(0,0,curphi)
                marker = Marker()
                marker.header.frame_id = "laser_frame"
                ...
                marker_array_msg.markers.append(marker)
                cnt+=1
            print("publish safe heading")
            temp_ang = np.linspace(-np.pi, np.pi, 360).transpose()

            self.danger_heading = delete_values(temp_ang,self.safe_heading)
                #initial value
            if len(self.danger_heading)!=0:

                for i in range(len(temp_ang)-1):
                    curphi = self.danger_heading[i]
                    color = [ 1.0,1.0,1.0]
                    curphi_q = euler_to_quaternion(0,0,curphi)
                    marker = Marker()
                    marker.header.frame_id = "laser_frame"
                    ...
                    marker_array_msg.markers.append(marker)
                    cnt+=1
            print("publish danger heading")
            self.viz_safe_heading_pub.publish(marker_array_msg)
