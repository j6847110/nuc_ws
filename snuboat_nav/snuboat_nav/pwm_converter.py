...
class PWMConverter(Node):
    ...
    def __init__(self):
        ...
        self.des_spd_sub =self.create_subscription(Float64, '/des_spd', self.des_spd_callback, 1)
        # self.OS_heading_sub = self.create_subscription(FilterHeading, '/nav/heading', self.OS_heading_callback, 1)
        self.spd_sub = self.create_subscription(Odometry, '/odometry/filtered', self.spd_callback, 1)    
    ...
    def check_topic_status(self):
        ...
        if self.des_heading_received and self.des_spd_received and self.OS_heading_received and self.spd_received:
    ...
    def des_spd_callback(self, msg):
        self.des_spd_received = True
        self.des_spd = np.append(self.des_spd, msg.data)
    ...
    def spd_callback(self, msg):
        ...
        temp_spd = [msg.twist.twist.linear.x,msg.twist.twist.linear.y,
                            msg.twist.twist.linear.z]
        spd = np.linalg.norm(temp_spd)
        self.spd = np.append(self.spd, spd)
        self.spd = self.spd[1:]
        
        err_spd = self.des_spd[-1] - self.spd[-1]
        # print(err_spd)
    ...
    def cal_thrust_pwm(self):
        #PD 
        thrust_pwm = (self.Kp_thrust * self.err_spd[-1]+ 
                      self.Kd_thrust *(self.err_spd[-1] - self.err_spd[-2]) / self.dt) + \
            (1500)
        print("this is error speed")
